...
const calculatePeriodMetrics = (periodData) => {
  ...
  const workoutDays = Object.values(dailyTotals).reduce(
    (sum, day) => sum + day.days,
    0
  );
...
const categorizeExercise = (exerciseName) => {
  ...
  if (/bench|chest|push[ -]?up|fly|press|dip/.test(exerciseName))
    return "Chest";
  if (/back|row|pull[ -]?up|pulldown|deadlift|lat/.test(exerciseName))
    return "Back";
  if (/squat|leg|calf|lunge|hamstring|quad|glute/.test(exerciseName))
    return "Legs";
  if (/shoulder|delt|military|overhead|press|shrug/.test(exerciseName))
    return "Shoulders";
  ...
  if (
    /run|jog|sprint|cardio|bike|cycling|treadmill|elliptical|rowing/.test(
      exerciseName
    )
  )
    return "Cardio";
...
export function WorkoutMetrics({
  ...
  const { color: volumeTrendColor, icon: volumeTrendIcon } =
    getTrendInfo(volumeTrend);
  ...
  const prepareDataByDate = () => {
    const groupedByDate = exerciseData.reduce((acc, entry) => {
      ...
      if (!acc[date].muscleGroups[muscleGroup])
        acc[date].muscleGroups[muscleGroup] = 0;
  ...
  const processExerciseData = () => {
    ...
    const { avgVolume, frequency, workoutDays, totalDays } =
      calculatePeriodMetrics(currentPeriodData);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <MetricCard
          ...
          goalText={`${workoutDays} workout ${
            workoutDays === 1 ? "day" : "days"
          } in this period`}
          color={VOLUME_COLOR}
        />
      </div>
